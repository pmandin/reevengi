...
#define TEXTURE_PIXEL_TYPE	Uint16
...
void draw_render_textured16_pc0opaquem68k(SDL_Surface *surf, Uint8 *dst_line, sbuffer_segment_t *segment, int x1,int x2)
{
	...
	if ((ubits+vbits>16) || !tex->paletted) {
		draw_render_textured16_pc0opaque(surf, dst_line, segment, x1, x2);
		return;
	}
...
void draw_render_textured16_pc1opaquem68k(SDL_Surface *surf, Uint8 *dst_line, sbuffer_segment_t *segment, int x1,int x2)
{
	...
	if ((ubits+vbits>16) || !tex->paletted) {
		draw_render_textured16_pc1opaque(surf, dst_line, segment, x1, x2);
		return;
	}
...
void draw_render_textured16_pc3opaquem68k(SDL_Surface *surf, Uint8 *dst_line, sbuffer_segment_t *segment, int x1,int x2)
{
	float u1,v1, u2,v2, du,dv, u,v;
	float w1, w2, w, dw;
	int dxtotal, dx, i;
	...
	if ((ubits+vbits>16) || !tex->paletted) {
		draw_render_textured16_pc3opaque(surf, dst_line, segment, x1, x2);
		return;
	}

...
static void re2ps1demo_load_bss_bg(const char *filename);
static void re2ps1demo_load_mdec_bg(void *buffer, int length);

/*--- Functions ---*/

void re2ps1demo_init(state_t *game_state)
{
	game_state->load_background = re2ps1demo_loadbackground;
	game_state->shutdown = re2ps1demo_shutdown;
}
...
void re2ps1demo_loadbackground(void)
{
	...
	re2ps1demo_load_bss_bg(filepath);

	free(filepath);
}

void re2ps1demo_load_bss_bg(const char *filename)
{
	SDL_RWops *src;
	Uint8 *dstBuffer;
	int dstBufLen;
	...
	if (dstBuffer && dstBufLen) {
		...
		re2ps1demo_load_mdec_bg(dstBuffer, dstBufLen);

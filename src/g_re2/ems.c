	...
	const re2ps1_ems_t **ems_array, int *emd_pos, int *tim_pos)
{
	char *filepath;
	int parsed = 0;
	int game_player, num_file;	
	const char *re2ps1_model_file = re2ps1_model;

	num_file = game_player = 0;	/* 0 for leon, 1 for claire */
	*filename = NULL;
	*ems_array = NULL;
	*emd_pos = -1;
	*tim_pos = -1;
	...
	/* Generate filename */
	filepath = malloc(strlen(re2ps1_model_file)+8);
	if (!filepath) {
		fprintf(stderr, "Can not allocate mem for filepath\n");
		return;
	}
	sprintf(filepath, re2ps1_model_file, game_player, num_file);
